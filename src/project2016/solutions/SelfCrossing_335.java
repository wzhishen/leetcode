...
public class SelfCrossing_335 {
    /* Ref: http://www.programcreek.com/2015/03/leetcode-self-crossing-java/
     *
     * Three cases in total when self crossing:
     * 1. X0 >= X2, X3 >= X1
     * 2. X0 + X4 >= X2, X1 = X3
     * 3. X1 + X5 >= X3, X0 + X4 >= X2,
     *    X1 <= X3, X5 <= X3,
    ...
    public boolean isSelfCrossing(int[] x) {
        ...
        for (int i = 3; i < x.length; ++i) {
            ...
            if (i >= 4 &&
                x[i-4] + x[i] >= x[i-2] && x[i-3] == x[i-1]) return true;
            if (i >= 5 &&
                x[i-4] + x[i] >= x[i-2] && x[i-5] + x[i-1] >= x[i-3] &&
                x[i-4] <= x[i-2] && x[i]   <= x[i-2] &&
                x[i-5] <= x[i-3] && x[i-1] <= x[i-3]) return true;
        }
        return false;
    }
}
