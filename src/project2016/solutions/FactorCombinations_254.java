package project2016.solutions;

import java.util.ArrayList;
import java.util.List;

/**
 * https://leetcode.com/problems/factor-combinations/
 *
 * Numbers can be regarded as product of its factors. For example,
 * 8 = 2 x 2 x 2;
 *   = 2 x 4.
 * Write a function that takes an integer n and return all possible
 * combinations of its factors.
 *
 * Note:
 * 1. Each combination's factors must be sorted ascending, for example:
 * The factors of 2 and 6 is [2, 6], not [6, 2].
 * 2. You may assume that n is always positive.
 * 3. Factors should be greater than 1 and less than n.
 *
 * Examples:
 * input: 1
 * output: []
 *
 * input: 37
 * output: []
 *
 * input: 12
 * output:
 * [
 *   [2, 6],
 *   [2, 2, 3],
 *   [3, 4]
 * ]
 *
 * input: 32
 * output:
 * [
 *   [2, 16],
 *   [2, 2, 8],
 *   [2, 2, 2, 4],
 *   [2, 2, 2, 2, 2],
 *   [2, 4, 4],
 *   [4, 8]
 * ]
 */
public class FactorCombinations_254 {
    public List<List<Integer>> getFactors(int n) {
        List<List<Integer>> lists = new ArrayList<List<Integer>>();
        if (n <= 0) return lists;

        getFactors(new ArrayList<Integer>(), lists, 2, n);
        return lists;
    }

    private void getFactors(List<Integer> list, List<List<Integer>> lists, int start, int n) {
        if (n == 1 && list.size() > 1) {
            lists.add(new ArrayList<Integer>(list));
        } else {
            for (int i = start; i <= n; ++i) {
                if (n % i == 0) {
                    list.add(i);
                    getFactors(list, lists, i, n / i);
                    list.remove(list.size() - 1);
                }
            }
        }
    }
}
